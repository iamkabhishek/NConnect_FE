...
import { buildApiUrl, getMe } from '../lib/api';
    ...
    const checkAuthAndLoad = async () => {
      ...
      if (effectiveToken) {
        try {
          ...
          const res = await fetch(buildApiUrl('/api/v1/helpdesk/tickets'), {
        ...
        if (urlCode && urlEmail) {
          try {
            const res = await fetch(buildApiUrl('/api/v1/helpdesk/tickets/guest/lookup'), {
              method: 'POST',
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        const res = await fetch(buildApiUrl(`/api/v1/helpdesk/tickets/${selectedTicketId}/messages`), { headers });
        const data = await res.json();
  ...
  const handlePostReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (token) {
        ...
        const res = await fetch(buildApiUrl(`/api/v1/helpdesk/tickets/${selectedTicketId}/messages`), {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
